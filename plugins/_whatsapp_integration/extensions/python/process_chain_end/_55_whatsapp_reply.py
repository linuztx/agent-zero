...
CTX_SEND_FAILURES: str = "_whatsapp_send_failures"
...
class WhatsAppAutoReply(Extension):
    ...
    async def _send_reply(
        ...
        error = await send_whatsapp_reply(context, response_text, attachments)
        if not error:
...
def _notify_agent_of_failure(
    ...
    msg = context.agent0.read_prompt(
        "fw.whatsapp.send_failed.md", error=error, attempt=str(attempt),
