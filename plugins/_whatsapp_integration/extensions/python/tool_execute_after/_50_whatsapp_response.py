...
from plugins._whatsapp_integration.helpers.handler import CTX_WA_HANDLER, CTX_WA_ATTACHMENTS
...
class WhatsAppResponseIntercept(Extension):
    ...
    async def execute(
        ...
        if not context.data.get(CTX_WA_HANDLER):
            return
    ...
    async def _send_inline(
        ...
        from plugins._whatsapp_integration.helpers.handler import send_whatsapp_reply
        ...
        error = await send_whatsapp_reply(context, text, attachments or None)

        if error:
            result = agent.read_prompt("fw.whatsapp.update_error.md", error=error)
        else:
            result = agent.read_prompt("fw.whatsapp.update_ok.md")
